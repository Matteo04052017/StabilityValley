...
public class SplineController : MonoBehaviour
{
    ...
    {
        if (gravSpline)
        {           //If the user has specified a gravity spline, set it up
    ...
    bool FindNextSpline(Vector3 position, Vector3 velocity, Vector3 acceleration, out Vector3 landing, out bool landed, out int traveltime)
    {
        ...
        if (predictionStep > 0)
        {
            ...
            if (x == 0)
            {       //This means that the first cast hit the collider
                for (int i = 0; i < predictionStep; i++)
                {
                    ...
                    if (Physics.Linecast(position, curr, out hit, 1 << SplineTopLayer))
                    {
                        ...
                        if (i == 0)
                        {   //This means that we'll land there this frame.
    ...
    protected Vector3 SplineMovement()
    {
        ...
        switch (mode)
        {
            ...
            case Mode.AUTO:
                ...
                {
                    if (horizFrameVelocity > 0 && groundNode.next)
                    {   //We're moving "forward" and there's a next vert
                    ...
                    {       //We're moving "backward" and there's a previous vert
        ...
        if (horizFrameVelocity != 0)
        {
            ...
            if (groundNode)
            {
                ...
                {
                    if (horizFrameVelocity > 0)
                    {       //Reaching the next vert going forward
                        if (!NextNode())
                        {
                            Detach();
                            return GetComponent<Rigidbody>().velocity;
                    ...
                    if (horizFrameVelocity < 0)
                    {   //Reaching the previous vert going backward
                        if (!PreviousNode())
                        {
                            Detach();
                            return GetComponent<Rigidbody>().velocity;
    ...
    protected void GravMovement(Vector3 position, out Vector3 gravPosition)
    {   //change the grav vert dependent on ground getPosition()
